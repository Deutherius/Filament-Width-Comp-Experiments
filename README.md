...
Note - I would not completely trust the X axis. I made a few prints that required 5m of filament but only got ~2200 log entries (I use ~~1 mm measurement_interval~~ not anymore, standard 10 mm is good enough). ~~I have yet to determine whether the logging is time based, time-limited, or what else is going on, so just... Take the X axis with a grain of salt~~ See appendix A.
...
Small update - It's been a few months and I can say that this compensation has given me zero issues. Everything just works, prints look great once the filament compensation kicks in (if the filament diameter is crap, otherwise there is not much change).

I have no idea about what accuracy/precision/repeatability to expect from the sensor, although the resolution seems to be about 1 micron. With good and consistent filaments, it is entirely possible that using the compensation would introduce more errors into the print than would otherwise be there, so use at your own risk*. Still, it makes my stash of esun ABS+ printable, so I am happy with it so far.
